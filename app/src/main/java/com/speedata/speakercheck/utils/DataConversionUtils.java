...
//		src.replace("\\s","");
	 ...
	 * @param bytes
	 * @return
	 */public static int byteArrayToInt(byte[] bytes ,boolean high_) {//false 小端,true大端
		int value = 0;
		// 由高位到低位   低位在前
		if(high_) {
			for (int i = 0; i < bytes.length; i++) {//正常大端
				int shift = (bytes.length - 1 - i) * 8;
				value += (bytes[i] & 0x000000FF) << shift;// 低位在前，往高位游
			}
		}else{
			for (int i = bytes.length-1; i >=0; i--) {//小端
				int shift = i * 8;
				value += (bytes[i] & 0x000000FF) << shift;// 高位在前，往低位游
			}
		}
		return value;
	}
